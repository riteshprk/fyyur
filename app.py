...
def venues():
    ...
    # Data from database
    unique_city_states = Venue.query.distinct(Venue.city, Venue.state)

    data1 = []
    for item in unique_city_states:
        unique_venue = Venue.query.filter(Venue.city == item.city)
        venue = []
        for item1 in unique_venue:
            venue.append(
                {'id': item1.id, 'name': item1.name})

        data1.append({'city': item.city, 'state': item.state, 'venues': venue})
...
def search_venues():
    ...
    for venue in venues:
        upcoming_show_count = venue.shows.filter(
            Show.show_time >= timenow).count()
        venue_data.append({
            "id": venue.id,
            "name": venue.name,
...
def show_venue(venue_id):
    ...
    venueupcomingshow = venue.shows.filter(Show.show_time >= timenow).all()

    venueupcomingshow_count = venue.shows.filter(
        Show.show_time >= timenow).count()

    venuepastshow = venue.shows.filter(Show.show_time < timenow).all()

    venuepastshow_count = venue.shows.filter(
        Show.show_time < timenow).count()

    def venuedata(queryobject):
        venue_data = []
        for row in queryobject:
            venue_data.append({
                "artist_id": row.artist.id,
                "artist_name": row.artist.name,
                "artist_image_link": row.artist.image_link,
                "start_time": str(row.show_time),
            })
        return venue_data
    ...
    dataq = {
        ...
        "past_shows": venuedata(venuepastshow),
        "upcoming_shows": venuedata(venueupcomingshow),
        "past_shows_count": venuepastshow_count,
        "upcoming_shows_count": venueupcomingshow_count,
    }

    return render_template('pages/show_venue.html', venue=dataq)
...
def create_venue_submission():
    ...
    if form.validate_on_submit():
        try:
            ...
            flash('Venue is not created due to error/s {}!!'.format(er))
            db.session.rollback()
        finally:
            db.session.close()
            return render_template('pages/home.html')
...
def search_artists():
    ...
    for artist in artists:
        upcoming_show_count = artist.shows.filter(
            Show.show_time >= timenow).count()
        artist_data.append({
            "id": artist.id,
            "name": artist.name,
...
def show_artist(artist_id):
    ...
    artistupcomingshow = artist.shows.filter(Show.show_time >= timenow).all()

    artistpastshow = artist.shows.filter(Show.show_time < timenow).all()

    def showdata(queryobject):
        show_data = []
        for row in queryobject:
            show_data.append({
    ...
    dataq = {
        ...
        "past_shows": showdata(artistpastshow),
        "upcoming_shows": showdata(artistupcomingshow),
        "past_shows_count": len(artistpastshow),
        "upcoming_shows_count": len(artistupcomingshow),
    }

    return render_template('pages/show_artist.html', artist=dataq)
...
def edit_artist(artist_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.commit()
            flash('Form edited successfully!!')
        except Exception as er:
            flash('Form didn\'t save due to error/s {}!!'.format(er))
            db.session.rollback()
        finally:
            db.session.close()
...
def edit_venue(venue_id):
    ...
    venue_query = Venue.query.filter(Venue.id == venue_id).first()
...
def create_artist_submission():
    ...
    if form.validate_on_submit():
        try:
            ...
            flash('Artist is not created due to error/s {}!!'.format(er))
            db.session.rollback()
        finally:
            db.session.close()
            return render_template('pages/home.html')
...
def create_show_submission():
    ...
    if form.validate_on_submit():
        try:
            ...
            flash('Show is not created due to error/s {}!!'.format(er))
            db.session.rollback()
        finally:
            db.session.close()
